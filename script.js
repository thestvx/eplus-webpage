...
(function initTestimonials() {
  const track = byId ? byId('testimonials-track') : document.getElementById('testimonials-track');
  const navEl = byId ? byId('testimonial-nav') : document.getElementById('testimonial-nav');
  if (!track || !navEl) return;

  let current = 0;
  const cards = () => [...track.querySelectorAll('.ep-testimonial-card')];
  const dots  = () => [...navEl.querySelectorAll('.ep-testimonial-dot')];

  function goTo(index) {
    const c = cards();
    if (!c.length) return;
    current = (index + c.length) % c.length;
  ...
  track.addEventListener('mouseleave', () => { timer = setInterval(() => goTo(current + 1), 6000); });
})();
...
function animateCounters() {
  document.querySelectorAll('.ep-stat-num[data-target]').forEach(el => {
...
/* ──────────────────────────────────────────────────────────
   SCROLL REVEAL + NAVBAR + COUNTER TRIGGER
────────────────────────────────────────────────────────── */
(function initScrollBehaviors() {
  const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (entry.target.classList.contains('ep-hero-stats-wrap') ||
            entry.target.querySelector?.('.ep-stat-num')) {
          animateCounters();
        }
  ...
  // Hamburger menu
  const hamburger = document.getElementById('ep-hamburger');
  const mobileMenu = document.getElementById('ep-mobile-menu');
  ...
  if (dot && outline && window.matchMedia('(pointer: fine)').matches) {
    ...
    document.querySelectorAll('a, button').forEach(el => {
      el.addEventListener('mouseenter', () => outline.style.transform = 'translate(-50%, -50%) scale(1.6)');
  ...
  if (pCanvas) {
    ...
    function drawParticles() {
      ...
      particles.forEach(p => {
        ...
        pCtx.fillStyle = `rgba(200,168,75,${p.a})`;
        pCtx.fill();
        p.x += p.vx; p.y += p.vy;
...
window.closeMobileMenu = function() {
  document.getElementById('ep-mobile-menu')?.classList.remove('open');
  document.getElementById('ep-hamburger')?.classList.remove('open');
};
