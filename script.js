...
// ─── DAYS ─────────────────────────────────────────────────
// maxDays: الحد الأقصى للأيام القابلة للاختيار (يُضبط ديناميكياً في VIP)
...
// ─── VIP: عدد الأيام ─────────────────────────────────────
window.onVipDaysCountChange = function() {
  ...
  maxDaysAllowed = val;
  // تحديث نص العداد
  const daysOf = document.getElementById('days-of-label');
  if (daysOf) daysOf.textContent = `/${val}`;
  // تحديث نص التعليمة
...
// ─── VIP EDU LEVEL ────────────────────────────────────────
window.onVipEduLevelChange = function() {
...
// ─── PENDING FORM DATA (للإرسال بعد الموافقة على القوانين) ──
...
function openTermsForSubmit(data) {
  ...
  if (!existingHint) {
    ...
    hint.innerHTML = `<span>⬇</span> <span>${currentLang==='ar'
      ? 'اقرأ القوانين كاملاً للمتابعة'
      : 'Scroll down to read all terms'}</span>`;
    document.querySelector('.terms-footer')
      .insertBefore(hint, document.querySelector('.terms-footer').firstChild);
  }

  // إخفاء نافذة التسجيل وإظهار القوانين
...
// زر "تأكيد التسجيل" في نافذة القوانين
window.proceedToRegister = async function() {
  ...
  overlay.innerHTML = `
    <div class="success-popup-box" id="success-popup-box">
      ...
      <h2 class="success-popup-title">
        ${currentLang==='ar' ? '🎉 تم استلام تسجيلك بنجاح' : '🎉 Registration Received'}
      </h2>
      <p class="success-popup-msg">
        ${currentLang==='ar'
          ...
          : `Thank you for joining <strong>E-PLUS Academy</strong>.<br>
             Your registration has been received and we'll contact you soon.<br>
...
// ─── FORM SUBMIT — يجمع البيانات ويفتح القوانين ────────────
window.submitForm = function(e) {
  ...
  // ✅ فتح نافذة القوانين مع حفظ البيانات
